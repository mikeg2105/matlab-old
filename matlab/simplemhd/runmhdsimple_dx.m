...
bx = 0.1*ones(max(size(x)),max(size(y)),2);
...
lh2=int32((45000/100000)*(max(size(y))-1)+1)
uh=int32(floor((55000/100000)*(max(size(y))-1)+1))
h(lh1:sh1,lh2:uh,1) = 50.30;
...
for na = 1:max(size(t))-1
  ...
  for i = 2:ni-1
    for j = 2:nj-1
       ...
       uhd=-((0.5*g)*(dt/dx))*(h(i+1,j,n)-h(i-1,j,n));
       vhd=-((0.5*g)*(dt/dy))*(h(i,j+1,n)-h(i,j-1,n));
 
      u(i,j,n+1) = (u(i+1,j,n)+u(i-1,j,n)+u(i,j+1,n)+u(i,j-1,n))/4-(0.5*(dt/dx))*u(i,j,n)*((u(i+1,j,n))-(u(i-1,j,n)))-((0.5*(dt/dy))*v(i,j,n))*(v(i,j+1,n)-v(i,j-1,n))+uhd-(1/rhobar)*((dt/(2.0*dx))*(p(i+1,j,n)-p(i-1,j,n)))+(1/(2*mu0*rhobar))*(by(i,j,n)*((dt/dy)*(bx(i,j+1,n)-bx(i,j-1,n)))-(dt/dx)*(by(i+1,j,n))-by(i+1,j));
    
    
    
      v(i,j,n+1) = (v(i+1,j,n)+v(i-1,j,n)+v(i,j+1,n)+v(i,j-1,n))/4-(0.5*(dt/dy))*v(i,j,n)*((v(i,j+1,n))/2-(v(i,j+1,n))/2)-((0.5*(dt/dx))*u(i,j,n))*(u(i+1,j,n)-u(i-1,j,n))+vhd+(1/rhobar)*((dt/(2*dy))*(p(i,j+1,n)-p(i,j-1,n)))+(1/(2*mu0*rhobar))*(bx(i,j,n)*((dt/dx)*(by(i+1,j,n)-by(i-1,j,n)))-(dt/dy)*(bx(i,j+1,n))-bx(i,j-1,n));
