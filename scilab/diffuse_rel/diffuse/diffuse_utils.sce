...
function mgendxgen(directory,jobname,nsteps, n1,n2,n3,nspecies)
  ...
  //write the form file
  filename=sprintf('%s/%s.form',directory,jobname);
  fd=mopen(filename, 'w');
  mfprintf(fd, '%d %d %d %d\n', nsteps, n1,n2,n3);
  mclose(fd);
  //write the general file for the form file
  ...
  //write the general file for the data
    filename=sprintf('tmp/%s.general',jobname);
        ...
        mfprintf(fd,"grid = %d x %d x %d\n",n1,n2,n3);
        mfprintf(fd,"format = ascii\n");
        mfprintf(fd,"interleaving = field\n");
        mfprintf(fd,"majority = row\n");
        mfprintf(fd,"header = lines 1\n");
        mfprintf(fd,"series= %d , 1, 1, separator= lines 1\n",nsteps);
        ...
        for i=1:nspecies
         ...
         mfprintf(fd,"dependency = ");
        for i=1:nspecies
          mfprintf(fd,"positions");
          if i<nspecies
            mfprintf(fd,",");
          else
            mfprintf(fd,"\n");
...
function mgendxgenmod(directory,jobname,nsteps, n1,n2,n3,nspecies)
  ...
  //write the general file for the data
    filename=sprintf('tmp/%s.general',jobname);
        ...
        //first form is for a 2d data set with n3=1
        //select function in dx fails!! if third dimension only 1 value
        if n3==1 then
          mfprintf(fd,"grid = %d x %d x %d\n",nsteps, n1,n2);
        else       
          mfprintf(fd,"grid = %d x %d x %d x %d\n",nsteps, n1,n2,n3);
        end  
        mfprintf(fd,"format = ascii\n");
        mfprintf(fd,"interleaving = field\n");
        mfprintf(fd,"majority = row\n");
        ...
        if n3==1 then
         ...
         mfprintf(fd,"positions = regular,regular,regular,regular, 0, 1, 0, 1, 0, 1, 0, 1\n");
        end
        mfprintf(fd,"end\n");  
        mclose(fd);
...
function  savemconcs(sdir, srootfilename,step, n1,n2,n3,nspecies, concs)

  
      sfilename=sprintf('%s/%s.dat', sdir,srootfilename);
      //sdir
      //sfilename
...
function [smat]=difreadmat(sfilename,nsteps,nspec)
...
mprintf('Reading concs\n');
  ...
  mval=zeros(nsteps,n1,n2,n3,nspec);
  for(t=1:nsteps)
  step=mfscanf(fd,'%f');
  fn1=mfscanf(fd, '%f');
  fn2=mfscanf(fd, '%f');
