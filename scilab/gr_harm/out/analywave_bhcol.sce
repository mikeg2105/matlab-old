...
//     Choose coeffs. depending on m value.
      ...
      if m==-2 then
         frr = 2*(sint.^2).*sinp.*cosp;
         frt = 2*sint.*cost.*sinp.*cosp;
         frp = sint.*(1-2.*sinp.^2);
         ftt1 = 2*(1+cost.^2).*sinp.*cosp;
         ftt2 = -2.*sinp.*cosp;
         ftp = -sint.*cosp; 
         fpp1 = -ftt1;
         fpp2 = 2*(cost.^2).*sinp.*cosp;
         drt = 8.*sint.*sinp.*cosp;
         drp = 4.*sint.*cost.*(1-2.*sinp.^2);
         dtt = -4.*cost.*sinp.*cosp;
         dtp = (2-sint.^2).*(2.*sinp.^2-1);
         dpp = 4.*cost.*sinp.*cosp;
      end
      ...
      if m==2 then
         ...
         frp = -2*sint.*sinp.*cosp;
         ftt1 = (1+cost.^2).*(1-2*sinp.^2);
         ftt2 = (2*sinp.^2-1);
         ftp = 2*cost.*sinp.*cosp;
         fpp1 = -ftt1;
         fpp2 = (cost.^2).*(1-2.*sinp.^2);
         drt = 4*sint.*(1-2*sinp.^2);
         drp = -8*sint.*cost.*sinp.*cosp;
         dtt = -2*cost.*(1-2*(sinp.^2));
         dtp = 2*(2-(sint.^2)).*sinp.*cosp;
