...
for n = 1:max(size(t))-1
  ...
  unew(max(size(x)),: ) = 2.5*unew(ni-1,: )-2*unew(ni-2,: )+0.5*unew(ni-3,: );
  unew(:,1 ) = 2.5*unew(:,2 )-2*unew(:,3 )+0.5*unew(:,4 );
  unew(:,max(size(y)) ) = 2.5*unew(:,nj-1 )-2*unew(:,nj-2 )+0.5*unew(:,nj-3 );

  vnew(1,: ) = 2.5*vnew(2,: )-2*vnew(3,: )+0.5*vnew(4,: );
  vnew(max(size(x)),: ) = 2.5*vnew(ni-1,: )-2*vnew(ni-2,: )+0.5*vnew(ni-3,: );
  vnew(:,1 ) = 2.5*vnew(:,2 )-2*vnew(:,3 )+0.5*vnew(:,4 );
  vnew(:,max(size(y)) ) = 2.5*vnew(:,nj-1 )-2*vnew(:,nj-2 )+0.5*vnew(:,nj-3 );

  hnew(1,: ) = 2.5*hnew(2,: )-2*hnew(3,: )+0.5*hnew(4,: );
  hnew(max(size(x)),: ) = 2.5*hnew(ni-1,: )-2*hnew(ni-2,: )+0.5*hnew(ni-3,: );
  hnew(:,1 ) = 2.5*hnew(:,2 )-2*hnew(:,3 )+0.5*hnew(:,4 );
  hnew(:,max(size(y)) ) = 2.5*hnew(:,nj-1 )-2*hnew(:,nj-2 )+0.5*hnew(:,nj-3 );
  //Write data to output
 mfprintf(fd, '%d %d %d\n',n, ni, nj);
 for j1=1:ni
  for j2=1:nj
      mfprintf(fd, '%f %f %f\n',unew(j1,j2),vnew(j1,j2),hnew(j1,j2)-5000);
      //mfprintf(fd, '%f\n',h(j1,j2,n)-5000);
  end
