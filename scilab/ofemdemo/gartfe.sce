...
FEel0(2:$,3)=12345; FEel0(2:$,4)=0; FEel0(2:$,7)=1e12;
femesh('addsel;');


// make a group of the part covered by the constraining layer
femesh('divide group 6 InNode {group6 & x>.55 & y<=.85 & y>=-.85}');

// tip masses
femesh('object mass',femesh('findnode y==0.93 | y==-0.93& x==0.42'));
femesh('addsel;');
FEelt(femesh('findeltgroup10'),2:4)= 0.2; 
...
pl = [1 1   72e9 0.34 2700  2.68e10
      2 1  210e9 0.29 7800  8.13e10];
il = [1 2     2 1 0     .01
      2 2     2 1 0     .016
      3 2     2 1 0     .05
      4 2     2 1 0     .011];
...
[m,k,mdof] = fe_mk(FEnode,FEel0,pl,il);
[md0,f0] = fe_eig(m,k,[1 20 1e2 11]);f0=f0/2/%pi;
feplot(FEnode,FEel0,md0,mdof,2);

//-----------------------------------------------------------------

//	Etienne Balmes  07/14/93, 07/13/00
//       Copyright (c) 1990-2000 by Etienne Balmes
//       All Rights Reserved.